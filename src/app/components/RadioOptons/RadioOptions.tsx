        ...
        <div className="mb-4 flex flex-col gap-4">       
            ...
            {options.map((option) => (
                <div className="flex flex-row gap-4" key={`${name}-${option.split(' ').slice(0, 2).join(' ')}`}>
                    <input {...register(name, { required: true })} id={option.split(' ').slice(0, 2).join(' ')} type="radio" className="border border-blueBlack border-2 p-2 rounded" />
                    <label className="text-sm" htmlFor={option.split(' ').slice(0, 2).join(' ')}>{option}</label>
                </div>
            ))}
        </div>
    )
