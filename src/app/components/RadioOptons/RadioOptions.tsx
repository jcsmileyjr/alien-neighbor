...
export default function RadioOptions({ name, label, options, errors, register }: { name: "user_name" | "user_illegal_name"| "user_commute_distance"| "user_run_speed", label: string, options: string[], errors: any, register: UseFormRegister<User> }) {
    return (
        <div className="mb-4 flex flex-col items-center gap-4">       
            <label className="text-base font-bold text-blueBlack gap-4" htmlFor={name}>{label} {errors[name] && <p className="text-sm text-red-500">Required</p>}</label>
            {options.map((option) => (
